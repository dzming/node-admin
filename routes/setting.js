...
router.post('/findU', async (req, res, next) => {
    let {
        inputEmail
    } = req.body
...
router.post('/update', async (req, res, next) => {
    ...
    let data = await update("goods", _id ? { _id: ObjectId(_id) } : {}, {
        $set: {
            name,
            age,
            skill,
...
router.post('/updates', async (req, res, next) => {
    ...
    let data = await update("user", _id ? { _id: ObjectId(_id) } : {}, {
        $set: {
            inputEmail,
            inputPassword
        }
...
router.post('/updatey', async (req, res, next) => {
    ...
    let data = await update("consumers", _id ? { _id: ObjectId(_id) } : {}, {
        $set: {
            uname,
            upwd
        }
...
router.post('/updatezt', async (req, res, next) => {
    ...
    let data = await update("goodslist", _id ? { _id: ObjectId(_id) } : {}, {
        $set: {
            numbers,
            condition
        }
